...
class DialogLoader(BaseModelXmlLoader[Dialog]):
    ...
    def _load(self, dialog_node: _Element, dialog_comments: list[str]) -> Dialog:
        ...
        for child_node in dialog_node:
            if child_node.tag == "precondition" and child_node.text:
                ...
                msg = f"Unexpected game dialog child {child_node.tag} in {dialog_id}"
                raise ValueError(
                    msg,
                )
    ...
    def _parse_dialog_phrase(  # noqa: C901 PLR0912
        ...
        try:
            ...
            msg = f"{dialog=}, {phrase_id=}"
            raise IntegrityError(msg) from ex
        _next: list[str] = []
        ...
        for child_node in phrase_node:
            if child_node.tag == "next" and child_node.text:
                ...
                msg = f"Unexpected dialog phrase child {child_node.tag} in {dialog.game_id} {phrase_id}"
                raise ValueError(
                    msg,
                )
